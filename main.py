...
class Vanguard:
        ...
        if open_ports:
            ...
            with ThreadPoolExecutor(max_workers=3) as executor:
                ...
                if ACTIVE_MODULES["nmap_aggressive"]:
                    future_aggressive_scan = executor.submit(
                        self.run_port_scanner_aggressive, open_ports
                    )

                # FFUF
                if ACTIVE_MODULES["ffuf"]:
                    future_fuzzing = executor.submit(
                        self.run_directory_fuzzer, web_targets
                    )

                    # WEB ANALYZER
                    if ACTIVE_MODULES["web_analyzer"]:
                        future_web_analysis = executor.submit(
                            self.run_web_analysis, list(web_targets.values())
                        )

                # SUBDOMAIN
                if ACTIVE_MODULES["subdomain"]:
                    future_subdomain_finding = executor.submit(
                        self.subdomain_finder.find_subdomains, target
                    )
                ...
                if ACTIVE_MODULES["nmap_aggressive"]:
                    ...
                    found_vulnerabilities = self.vuln_searcher.search_for_exploits(
                        services_to_scan
                    )
                else:
    ...
    def run_port_scanner_aggressive(self, ports: list[str]):
        return PortScanner().parse_nmap_aggressive_xml(
            PortScanner().full_scan(self.target, ports=ports)
        )

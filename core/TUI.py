...
class TUI:
    ...
    def construct_scan_scene(self) -> None:
        scan_scene = Scene()
        scan_scene.add_widget(Label(x=10, y=5, text=f"Scanning ...", color="31"))
        self.scene_manager.add_scene("scan_scene", scan_scene)
        self.scene_manager.switch_scene("scan_scene")
    
    def construct_results_scene(self, open_ports: dict[str, str], fuzzing_output: list[str]) -> None:
        results_scene = Scene()
        ...
        for port, service in open_ports.items():
            ...
            y_offset += 1
        results_scene.add_widget(Label(x=10, y=y_offset + 1, text="Fuzzing Output:", color="32"))
